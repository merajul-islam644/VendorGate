...
export function RepoBrowserPage() {
  ...
  return (
    <section className="rb-page">
      ...
      <div className="rb-clone-row">
        <label className="rb-field rb-field-grow rb-clone-field">
          <span>Clone by URL</span>
          <div className="rb-clone-input-wrap">
            <input
              ref={cloneInputRef}
              value={cloneInput}
      ...
      <nav className="rb-breadcrumbs" aria-label="Path">
        <button
          type="button"
          className="rb-crumb rb-crumb-home"
        ...
        {breadcrumbs.map((crumb, index) => (
          <span key={crumb.path} className="rb-crumb-row">
            <ChevronRight size={12} />
            <button
              type="button"
              className={`rb-crumb${index === breadcrumbs.length - 1 ? " rb-crumb-active" : ""}`}
      ...
      <div className="rb-grid">
        <aside className="rb-tree-card">
          ...
          {state.kind === "loading" && tree.length === 0 ? (
            ...
            <div className="rb-empty">Path not found in tree.</div>
          ) : entries.length === 0 ? (
            <div className="rb-empty">Empty directory.</div>
          ) : (
              ...
              {entries.map((entry) => {
                ...
                return (
                  <li key={entry.path}>
                    <button
                      type="button"
                      className={`rb-entry rb-entry-${entry.type}${isOpen ? " rb-entry-active" : ""}`}
          ...
          <header className="rb-card-header">
            ...
            {selected ? (
              <>
                {dirty ? (
                  <span
                    className="rb-pill rb-pill-dirty"
                    title="This file has unsaved local edits."
                  >
                    <CircleAlert size={11} /> Modified
                  </span>
                ) : null}
                <span className="rb-card-meta">{humanSize(selected.size)}</span>
                <button
                  type="button"
                  className="rb-discard"
      ...
      {playwrightRepoActive && selected && blocks.length > 0 ? (
        <aside className="rb-outline-card">
          <header className="rb-card-header">
            <ListChecks size={14} />
            <span>
          ...
          <ul className="rb-outline">
            {blocks.map((block, idx) => (
              ...
              >
                ...
                <span className="rb-outline-name" title={block.source}>
                  {block.name}
                </span>
                <button
                  type="button"
                  className="rb-outline-play"
      ...
      <div
        className={`rb-env-drawer${envExpanded ? " rb-env-drawer-open" : ""}`}
        role="dialog"
        aria-modal="true"
      ...
      >
        <button
          type="button"
          className="rb-env-backdrop"
          tabIndex={envExpanded ? 0 : -1}
          aria-label="Close secrets panel"
          onClick={() => setEnvExpanded(false)}
        />
        <aside className="rb-env-panel">
          <header className="rb-env-header">
            <KeyRound size={14} />
            ...
            <div className="rb-env-header-actions">
              <button
                type="button"
          ...
          <div className="rb-env-body">
            ...
            */}
            <section className="rb-token-section">
              <header className="rb-token-section-header">
                <KeyRound size={14} />
              ...
              <div className="rb-token-row">
                ...
                {githubTokenFilled ? (
                  <button
                    type="button"
                    className="rb-env-toggle rb-token-clear"
                    onClick={() => {
                      setGithubToken("");
              ...
              <span className="rb-env-hint rb-token-hint">
            ...
            <div className="rb-env-inputs">
              <div className="rb-env-input-row">
                <label className="rb-env-input">
                  <span>Key</span>
                  <input
                    value={draftKey}
                ...
                <label className="rb-env-input rb-env-input-grow">
                  <span>Value</span>
                  <input
                    value={draftValue}
              ...
              <div className="rb-env-actions">
                <button
                  type="button"
                  className="rb-env-add"
            ...
            {envEntries.length > 0 ? (
              <ul className="rb-env-list">
                {envEntries.map((entry) => (
                  <li key={entry.id} className="rb-env-row">
                    <input
      ...
      */}
      <div
        className={`rb-run-drawer${inlineDrawerOpen ? " rb-run-drawer-open" : ""}${
          inlineRunning ? " rb-run-drawer-running" : ""
        }${inlineDrawerCollapsed ? " rb-run-drawer-collapsed" : ""}`}
        role="dialog"
        aria-modal="false"
